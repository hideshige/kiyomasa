...
class image {
   ...
   */
  public static function thumbnail($file, $folder_name, $scan_flag = true, $width_max = 160, $height_max = 160, $comp = 98, $file_type = null, $set_name = '', $file_no = 0, $p = array('255','255','255'), $limit = 104857600) {
    //ウィルススキャン
    //if ($scan_flag) self::virus_scan($file);

    //サイズとファイル形式のチェック
    self::_size_check($file, $limit);
    $itype = self::_file_type($file, $file_type);
    ...
    if (!$img_res) { throw new Exception('thumbnail resample error'); }

    $name = $set_name ? ($file_type ? $set_name . '.' . $itype : $set_name) : md5(TIMESTAMP . S::$user['user_id']) . $file_no . '.' . $itype;
    ...
    if (!$save) throw new Exception('thumbnail save error');
   ...
   * @param boolean $scan_flag ウィルススキャンするかどうか
   */
  public static function up_file($file, $save_folder_file, $file_type = '', $limit = 104857600, $scan_flag = true) {
    //if ($scan_flag) self::virus_scan($file);
    self::_size_check($file, $limit);
    $itype = self::_file_type($file, $file_type);
    if (!$file_type) { $save_folder_file .= '.' .  $itype; }
    $res = move_uploaded_file($file, $save_folder_file);
    if (!$res) { throw new Exception('file move error'); }
  }
   ...
   */
  public static function virus_scan($file) {
    //ウィルスチェック（ウィルスだった場合ファイルを削除する）
    $do = sprintf('clamdscan "%s" --remove --log=%slogs/antivirus_%s.log', $file, SERVER_PATH, date('Ymd'));
    $debug = '';
    exec($do, $debug);
    chmod(sprintf('%slogs/antivirus_%s.log', SERVER_PATH, date('Ymd')), 0644);

    //ファイルが削除されている場合false
    if (!file_exists($file)) { throw new Exception('virus file deleted'); }
  }
   ...
   * @return string
   */
  private static function _file_type($file, $file_type) {
    if (!$file_type) {
      //MIMEタイプを調べる
   ...
   * @param integer $limit 制限サイズ
   */
  private static function _size_check($file, $limit) {
    $file_byte = filesize($file);
    if ($file_byte > $limit) { throw new Exception('file size over'); }
  }
}
