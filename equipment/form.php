...
if (isset (S::$post['post_flag'])) {
  ...
  if ($vali) {
    ...
    form::get_message($vali, $form_list);
    return true;
  }
}
 *
...
class form {
   ...
   */
  public static function validation($params, $data) {
    self::$_params = $params;
    $res = array ();
   ...
   */
  public static function get_message($error_data, &$form_list, $error_customize = array ()) {
    $k = 0;
    foreach (self::$_params as $name => $v) {
      if (isset ($v['join'])) {
        $k --;
      }
      if (isset ($error_data[$name])) {
        ...
        if ($error_customize) {
          foreach ($error_customize as $eck => $ecv) {
            if (array_key_exists($eck, $error_data[$name])) $m .= $ecv;
          }
        }
        if (array_key_exists('must', $error_data[$name])) $m .= self::$_e['must'];
        if (array_key_exists('must_select', $error_data[$name])) $m .= self::$_e['must_select'];
        if (array_key_exists('max', $error_data[$name])) $m .= sprintf(self::$_e['max'], $v['max'], $error_data[$name]['max']);
        if (array_key_exists('min', $error_data[$name])) $m .= sprintf(self::$_e['min'], $v['min'], $error_data[$name]['min']);
        if (array_key_exists('fill', $error_data[$name])) $m .= sprintf(self::$_e['fill'], $v['fill'], $error_data[$name]['fill']);
        if (array_key_exists('int', $error_data[$name])) $m .= self::$_e['int'];
        if (array_key_exists('eng', $error_data[$name])) $m .= self::$_e['eng'];
        if (array_key_exists('char', $error_data[$name])) $m .= self::$_e['char'];
        if (array_key_exists('hiragana', $error_data[$name])) $m .= self::$_e['hiragana'];
        if (array_key_exists('katakana', $error_data[$name])) $m .= self::$_e['katakana'];
        if (array_key_exists('email', $error_data[$name])) $m .= self::$_e['email'];
        if (array_key_exists('email2', $error_data[$name])) $m .= self::$_e['email2'];
        if (array_key_exists('email3', $error_data[$name])) $m .= self::$_e['email3'];
        if (array_key_exists('count', $error_data[$name])) $m .= sprintf(self::$_e['count'], $error_data[$name]['count']);
        if (array_key_exists('match_count', $error_data[$name])) $m .= sprintf(self::$_e['match_count'], $error_data[$name]['match_count']);
        if (array_key_exists('checkdate', $error_data[$name])) $m .= self::$_e['checkdate'];
        if (array_key_exists('select_error', $error_data[$name])) $m .= self::$_e['select_error'];
        if (array_key_exists('not_match1', $error_data[$name])) $m .= self::$_e['not_match1'];
        if (array_key_exists('not_match2', $error_data[$name])) $m .= self::$_e['not_match2'];
        if (array_key_exists('password_error', $error_data[$name])) $m .= self::$_e['password_error'];
        if (array_key_exists('password_error2', $error_data[$name])) $m .= self::$_e['password_error2'];
        if (array_key_exists('ng_word', $error_data[$name])) $m .= self::$_e['ng_word'];
      }
      $k ++;
    }
  }
   ...
   */
  public static function form_arr($params, &$form, $post, $br_flag = false) {
    foreach ($params as $name => $val) {
      ...
      if ($val['type'] == 3) {
        self::_select_form($name, $key, $val['select'], @$post[$name], $form, @$val['tag_add']);
      } else if ($val['type'] == 2) {
        self::_radio_form($name, $key, $val['select'], @$post[$name], $form, @$val['tag_add'], $br_flag);
      } else if ($val['type'] == 4) {
        self::_check_form($name, $key, $val['select'], @$post[$name], $form, @$val['tag_add'], $br_flag);
      } else if ($val['type'] == 5) {
        if (!isset ($val['size'])) $val['size'] = 10;
        $max = @$val['max'] ? sprintf(' maxlength="%d"', $val['max']) : '';
        $t = '<input type="text" name="%s" value="%s" size="%d"%s%s />';
        $form[$key] .= sprintf($t, $name, @$post[$name], $val['size'], $max, @$val['tag_add']);
      } else if ($val['type'] == 6) {
        if (!isset ($val['size'])) $val['size'] = 10;
        $max = @$val['max'] ? sprintf(' maxlength="%d"', $val['max']) : '';
        ...
        $form[$key] .= sprintf($t, $name, @$val['cols'] ? $val['cols'] : 50, @$val['rows'] ? $val['rows'] : 4, @$val['tag_add'], @$post[$name]);
      }
      if (isset ($val['add'])) {
        $form[$key] .= $val['add'];
      }
   ...
   */
  private static function _select_form($name, $key, $val, $data, &$form, $tag_add = '') {
   ...
   */
  private static function _radio_form($name, $key, $val, $data, &$form, $tag_add = '', $br_flag = false) {
    if (!isset ($form[$key])) $form[$key] = '';
    foreach ($val as $k => $v) {
   ...
   * @param boolean $br_flag 改行を入れるか否か
   */
  private static function _check_form($name, $key, $val, $data, &$form, $tag_add = '', $br_flag = false) {
    if (!isset ($form[$key])) $form[$key] = '';
    foreach ($val as $k => $v) {
      if (!isset($data)) $data = array ();
      if (!is_array ($data)) $data = explode(',', $data);//区切り文字の場合は配列に変換
      $check = (array_search($k, $data, false) !== false) ? ' checked="checked"' : '';
      $br = $br_flag ? '<br />' : '';
   ...
   */
  public static function disp_confirm(&$form, $question, $answer) {
  ...
  public static function make_data(&$form, $question, $answer) {
    foreach ($question as $k => $v) {
